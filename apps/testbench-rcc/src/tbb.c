...
tbb_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        TBB_LOG(INFO, "Security IO event; conn_handle=%d action=%d",
                event->passkey.conn_handle, event->passkey.params.action);
        if (event->passkey.params.action != BLE_SM_IOACT_OOB) {
            TBB_LOG(INFO, "; ignoring\n");
            return 0;
        }
        TBB_LOG(INFO, "\n");

#if MYNEWT_VAL(RCC_BLE_OOB_SECRET)
        rc = rcc_ble_inject_oob(event->passkey.conn_handle);
        TBB_LOG(INFO, "injected OOB key; rc=%d\n", rc);
#else
        TBB_LOG(INFO, "unsupported (RCC_BLE_OOB_SECRET disabled)\n");
#endif
        return 0;
